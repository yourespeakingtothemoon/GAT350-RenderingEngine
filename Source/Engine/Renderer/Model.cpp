...
namespace nc {
	...
	void Model::Draw(GLenum primitive) {
		...
		this->vertexBuffer->Draw(primitive);
	}
	...
	void Model::ProcessMesh(aiMesh* mesh, const aiScene* scene, const glm::mat4& transform) {
		...
		this->vertexBuffer->CreateVertexBuffer((GLsizei) (sizeof(vertex_t) * vertices.size()), (GLsizei) vertices.size(), vertices.data());
		this->vertexBuffer->SetAttribute(0, 3, sizeof(vertex_t), 0);
		this->vertexBuffer->SetAttribute(1, 2, sizeof(vertex_t), offsetof(vertex_t, texcoord));
		this->vertexBuffer->SetAttribute(2, 3, sizeof(vertex_t), offsetof(vertex_t, normal));
		this->vertexBuffer->SetAttribute(3, 3, sizeof(vertex_t), offsetof(vertex_t, tangent));
		...
		this->vertexBuffer->CreateIndexBuffer(GL_UNSIGNED_INT, (GLsizei) indices.size(), indices.data());
	}
}